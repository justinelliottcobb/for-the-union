...
export function runTests(compiledCode: string): TestResult[] {
  ...
  try {
    // Create a test module from the compiled code
    const testModule = new Function('require', 'module', 'exports', 'React', compiledCode);
    ...
    const mockRequire = (name: string) => {
      ...
      if (name === '@tanstack/react-query') return {
        ...
        useInfiniteQuery: () => ({ data: null, fetchNextPage: () => {}, hasNextPage: false }),
        useQueryClient: () => ({}),
        ReactQueryDevtools: () => {}
      };
      return {};
    ...
    testModule(mockRequire, module, module.exports, { 
      useState: () => [null, () => {}],
      useEffect: () => {},
      useCallback: () => {},
      useRef: () => ({ current: null })
    ...
    const componentResult = createComponentTest(
      'App component renders correctly with QueryClientProvider',
      compiledCode,
      'App'
    );
    ...
    results.push({
      ...
      passed: false,
      message: `Failed to compile code: ${error instanceof Error ? error.message : 'Unknown error'}`
    });
  }
