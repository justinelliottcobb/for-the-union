...
export function runTests(compiledCode: string): TestResult[] {
  ...
  try {
    ...
    const mockRequire = (name: string) => {
      ...
      if (name === 'zod') return {
        z: {
          ...
          string: () => ({ min: () => ({}), email: () => ({}), url: () => ({ optional: () => ({}) }) }),
          array: () => ({ parse: (data: any) => data })
        }
      };
      return {};
    };

    testModule(mockRequire, module, module.exports, { useState: () => [null, () => {}] });
    ...
    const componentResult = createComponentTest(
      'ApiClientDemo component renders correctly',
      compiledCode,
      'ApiClientDemo'
    );
